...
class EvtLinkTest {
    ...
    void checkMaxAmountOfEveryPay() {
        ...
        final EvtLink.Segment segment =
                parsedLink.getSegments().stream().filter(s -> s.getTypeKey() == 43).findFirst().orElse(null);
        ...
        final EvtLink.Segment segment2 =
                parsedLink2.getSegments().stream().filter(s -> s.getTypeKey() == 94).findFirst().orElse(null);

        if (segment2 != null) {
            Assertions.assertEquals(94, segment2.getTypeKey());
            Assertions.assertEquals(Long.toString(maxAmount2), new String(
                    segment2.getContent(),
